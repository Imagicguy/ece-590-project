...
# Input: A(an array), i(start index of current array(inclusive)), j(end index of current array(inclusive)) 
# Output: A(a sorted array)
Sort a list A with the call QuickSort(A, 0, len(A)).

def helper(A,i,j):
    if j - i < 1:# base case: if the length of current array is 1,not need to sort 
        return A
    pivot = A[j] # choose the right end element of current array as the pivot
    left = i # use two pointers ,use right to traverse all element in current array
    right = j - 1 # use left to track boundary of sorted elements
    while left < right:
        if A[right] >= pivot:
            right-=1
        else:
            A[left] ,A[right] = A[right], A[left] # put all elements smaller than pivot to the  left side of current array
            left+=1
    if A[left] < pivot:
        left+=1
    A[j],A[left] = A[left], A[j]
    helper(A,i,left -1) # divide current array by the pivot
    helper(A,left + 1,j) # conquer two children array by recurively calling helper function
    return A
